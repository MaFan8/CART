...
if($_SERVER['REQUEST_METHOD'] == 'GET' && isset($_GET["select-query"]))
{
  ...
  if($_GET["select-query"] =="default"){
    ...
    if($_GET["select-query"]==="four"){
      //TO IMPLEMENT:
        $selectedQuery = "SELECT * FROM dataStuff GROUP BY";
      }

    if($_GET["select-query"]==="five"){
      //TO IMPLEMENT:
      $selectedQuery = "SELECT * FROM dataStuff WHERE days = Monday OR days = Tuesday ORDER BY event_affect_strength";
    }
  ...
  if($selectedQuery!==""){
      ...
      // if it is the fourth query add on events at end of outArr
      if($_GET["select-query"] =="four"){
        $outArr[]= $events;
      }
      // if it is the fifth query add on events at end of outArr
      if($_GET["select-query"] =="five"){
        $outArr[]= $days;
      }
